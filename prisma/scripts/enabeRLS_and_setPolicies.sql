...
ALTER TABLE quest_pools ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read for everyone" ON quest_pools FOR SELECT USING (true);
CREATE POLICY "Only agora-labs.com staff can insert"
ON quest_pools FOR INSERT
TO authenticated
WITH CHECK (right(auth.jwt() ->> 'email', 15) LIKE '@agora-labs.com');
...
ALTER TABLE snapshot_data ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow read for everyone" ON snapshot_data FOR SELECT USING (true);
CREATE POLICY "Only agora-labs.com staff can insert"
ON snapshot_data FOR INSERT
TO authenticated
WITH CHECK (right(auth.jwt() ->> 'email', 15) LIKE '@agora-labs.com');
